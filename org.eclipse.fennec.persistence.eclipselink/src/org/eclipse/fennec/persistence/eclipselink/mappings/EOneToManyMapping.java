...
/**
 * {@link OneToManyMapping} variant that is aware of EMF lazy proxies (AP-47).
 * <p>
 * Provides the ID-only target-table query (the FK lives in the target table, so only
 * the target's primary-key column is selected — no object materialisation) and skips
 * unresolved EMF proxies in the commit-time cascade traversals.
 *
 * @author Mark Hoffmann
 * @since 15.07.2026
 */
public class EOneToManyMapping extends OneToManyMapping implements EMFConfigurableMapping, ETargetIdQuerySupport {

	/** serialVersionUID */
	private static final long serialVersionUID = 826350002926867298L;
