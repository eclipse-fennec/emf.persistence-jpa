...
Options:
  ...
  --project <id>        Eclipse project id, e.g. modeling.fennec
                                                (env: DASH_PROJECT_ID)
...
# ---- 2) run dash-licenses -------------------------------------------------
# The Eclipse Foundation license endpoint 504s on large batches (the default is
# 1000, this workspace has ~290 deps in one query) — keep batches small.
DASH_ARGS=("$DEPS" -summary "$SUMMARY" -batch "${DASH_BATCH:-50}")
if [[ "$REVIEW" == true ]]; then
