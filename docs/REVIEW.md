...
| AP-46 | **Echtes Lazy-Loading für Non-Containment-Refs (DB-Fetch deferred):** Baut auf AP-45 auf, greift aber tiefer in die EclipseLink-Indirection-Pipeline ein. Ziel: Beim `resource.load()` werden Non-Containment-Ziele **nicht** mitgeladen; nur der FK-Wert aus der Source-Row wird gelesen und daraus direkt ein Proxy konstruiert. Echter DB-Call erst bei `eGet`. **Scope:** (1) `ReferenceConfigurator.setMappingDefaults` → `setIsLazy(!reference.isContainment())`, (2) neue oder erweiterte IndirectionPolicy, die den FK-Wert aus dem nicht-instanziierten `QueryBasedValueHolder` (bzw. dessen Row-Daten) liest **ohne** `vh.getValue()` zu triggern, (3) daraus Proxy bauen (wie in AP-45, aber ohne vorher das Ziel zu materialisieren), (4) `EReferenceAccessor.unwrapValueHolder` Read-/Write-Kontext-Trennung, damit cascade-persist/merge nicht versehentlich Proxies statt echter Entities bekommt, (5) Entscheidung: `em.find`-Pfad ohne ResourceSet — akzeptieren dass der Proxy dort unaufgelöst bleibt, oder zweiten Auflösungs-Fallback über die EclipseLink-Session anbieten. **Risiko:** EclipseLink-Interna (`QueryBasedValueHolder.getRow()` ist nicht Teil der stabilen API), Regressionen in Bidirektional- und Cascade-Pfaden. **Testnachweis:** Erweiterung von `NonOsgiCrossResourceRefTest` um Assertion, dass `resource.load()` *keine* zusätzlichen Queries für das Ziel-Entity auslöst (über EclipseLink-Session-Log oder JDBC-Spy); Proxy-Auflösung löst dann den einen DB-Call aus. | Performance/Skalierung | L | ❌ Offen |
...
**Gesamtstand: 41 von 46 Arbeitspaketen erledigt (89 %)**
...
**Welle 21 — Erweiterungen:** AP-24, AP-31, AP-38, AP-46 (echtes Lazy-Loading)
