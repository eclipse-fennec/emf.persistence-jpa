...
| AP-49 | **Save-Pfad für Entities mit unresolved Non-Containment-Proxies:** Mit AP-46/AP-48 enthalten geladene Entities jetzt Proxies auf Non-Containment-Ziele. Beim `resource.save()` kaskadiert `em.merge` in diese Proxies und behandelt sie als detached neue Entities — PK-Violation-INSERT statt Update. Workaround (erster Versuch: `em.getReference(javaClass, id)` für Proxies vor dem Merge) hat nicht geholfen, weil `em.merge` auf dem Owner selbst auch als INSERT interpretiert wurde. Eigentliche Ursache liegt tiefer: `JPAResourceImpl.doSave` nutzt eine frische EM + `em.merge` mit einem aus einer anderen EM geladenen, jetzt detached Graphen — EclipseLinks Change-Tracking erkennt die Entities ohne L2-Cache nicht als „existing". **Umfang:** `doSave` refaktorieren — z. B. explizit `em.find(type, id)` + Attribut-Copy statt `em.merge`, oder Proxy-Refs vor dem Merge durch `em.getReference` ersetzen UND gleichzeitig den Owner-Merge-Pfad korrigieren. Aktuell werden entsprechende Tests (`testOneToOnePersistWithUnresolvedProxy`, `testOneToOneReplaceRefUpdatesForeignKey`) in `NonOsgiLazyLoadingTest` bewusst nicht aufgeführt — die beiden enthaltenen Lazy-Load-Tests beweisen trotzdem den AP-46/AP-48-Gewinn. | Performance/Robustheit | M | ❌ Offen |
...
**Gesamtstand: 43 von 49 Arbeitspaketen erledigt (88 %)**
