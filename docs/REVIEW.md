...
| F8-01 | F8 | ~~Kein automatisches Change-Tracking~~ **Evaluiert (AP-19)** — Bewusste Design-Entscheidung: Explizites `merge()` via `resource.save()` beibehalten. | — | ~~EContentAdapter-Brücke~~ |
...
| AP-19 | **Change-Tracking-Brücke evaluiert:** EContentAdapter-Ansatz analysiert und verworfen (Notification-Overhead beim Laden, Detached-Entity-Problem, Non-Containment-Lücke, Paradigmen-Mismatch). Bewusste Design-Entscheidung: Explizites `merge()` via `resource.save()` beibehalten — Standard-JPA-Pattern, performant, vollständig getestet. | F8-01 | M | ✅ Done |
...
| AP-38 | **Erweiterte Dokumentation:** Neue `docs/getting-started.md` (354 Zeilen) mit vollständigem Library-Beispielprojekt — Ecore-Modell, Non-OSGi-Bootstrap, OSGi-DS-Bootstrap, CRUD via `jpa://` Resource, Pagination, Lazy-Loading-Semantik inklusive „EMF Resource, backed by JPA"-Positionierung und `em.find`-Escape-Hatch-Hinweis. Neue `docs/configuration-reference.md` (143 Zeilen) mit vollständigem Property-Katalog: OCD-Properties, forwarded EclipseLink-Keys inkl. Defaults und Override-Policy, Resource-Load/Save-Options (`OPTION_PAGE_SIZE`, `OPTION_CACHE_NEW_OBJECTS`, `OPTION_TABLE_NAME`, `READ_FILTER_ECLASS`), Non-OSGi-Bootstrap-Properties. README aktualisiert mit Links zu beiden neuen Dokumenten. | F15-02, F15-03 | M | ✅ Done |
...
| AP-46 | **Echtes Lazy-Loading für Non-Containment-Refs (DB-Fetch deferred):** (1) `ReferenceConfigurator.setMappingDefaults` → `setIsLazy(!reference.isContainment())`. (2) `EBasicIndirectionPolicy.valueFromQuery(..., sourceObject, session)` + `(..., session)` überschrieben: liest FK aus der Source-`AbstractRecord` via `ObjectReferenceMapping.extractPrimaryKeysForReferenceObjectFromRow(row)` — kein DB-Call. Erzeugt EclipseLink-managed Dynamic-Proxy via `targetDescriptor.getInstantiationPolicy().buildNewInstance()`, setzt ID-Attribut und `eSetProxyURI(jpa://puName/TargetEClass#//refName/idAttr/idValue)`, gibt den Proxy direkt zurück statt eines `ValueHolder`s. CacheId-Unwrap für Single-PK. (3) `validateAttributeOfInstantiatedObject` wickelt EMF-Proxies nicht mehr in VHs ein (würde die `eIsProxy()`-Semantik kaputt machen). `getRealAttributeValueFromObject`, `cloneAttribute`, `extractPrimaryKeyForReferenceObject` akzeptieren jetzt nackte Proxy-EObjects statt nur VH. (4) AP-45-Post-Load-Proxifizierung (`JPAResourceImpl.proxifyNonContainmentRefs`) komplett entfernt — Proxies kommen jetzt direkt aus der Indirection-Pipeline. (5) `em.find`-basierte Tests (OneToOne/OneToMany Non-Containment in NoCache-Varianten) auf Resource-Load umgeschrieben; neuer Test-Base-Helper `findViaResource(rs, entityName, id)` teilt ResourceSet für Cross-Resource-Identity; Bidi-Assertions nutzen `EcoreUtil.getID(...)` statt Java-Identität (da jeder `em.find` in `JPAResourceImpl.getEObject` eine frische EM öffnet). D's aus Lazy-OneToMany-Collections haben keine `eResource()`, deshalb wird `EcoreUtil.resolve(proxy, rs)` explizit für ihre Back-Refs verwendet. (6) Neuer **`testLazyNonContainmentDefersTargetQuery`**: `SessionEventAdapter` zählt ReadObject/ReadAll-Queries. Beweis: `aResource.load()` löst genau **1** Query aus (nur ClassAO2O), `a.eGet(dNonContainment)` löst die Target-Query aus, zweiter `eGet` tut **nichts** (Proxy ist resolved). Build: **94 Non-OSGi + 22 OSGi Tests grün**. | Performance/Skalierung | L | ✅ Done |
| AP-47 | **Element-Level-Proxies für Non-Containment-Collections:** Erweitert AP-46/48 auf alle mehrwertigen Non-Containments. `ETransparentIndirectionPolicy` ersetzt die Standard-Selection-Query durch eine ID-only `DirectReadQuery`, die das Mapping selbst liefert (`ETargetIdQuerySupport`): Relation-Table-Variante (M2M, uni-O2M mit JoinTable) liest nur die Join-Table; FK-in-Target-Variante (bidi O2M) liest nur die PK-Spalte der Target-Tabelle. Pro ID wird ein EMF-Proxy (`id` + `eProxyURI`) gebaut — Targets werden beim Listen-Füllen nie materialisiert; Auflösung via ResourceSet → `JPAResourceImpl.getEObject`. **Refactoring (SoC):** EMF-aware Mapping-Klassen `EOneToOneMapping`/`EManyToOneMapping`/`EOneToManyMapping`/`EUnidirectionalOneToManyMapping`/`EManyToManyMapping` mit gemeinsamem `EMappingSupport`; EMF-Konfiguration (Accessor, Indirection-Policy, Defaults) wandert via `EMFConfigurableMapping.configureEMF` aus dem `ReferenceConfigurator` in die Mappings; alle Typen überspringen unaufgelöste Proxies in den Cascade-Persist-Traversalen. Begleitende Fixes: `EObjectBuilder.buildBackupClone` snapshottet Proxy-Collections ins Backup; `cloneAttribute` proxifiziert materialisierte Werte statt Shared-Cache-Originale durchzureichen; `EReferenceAccessor` füllt Collections framework-intern via `basicAdd` (ohne EMF-Opposite-Pflege — single-valued Opposites würden sonst Elemente zwischen Clone/Backup „stehlen") und dedupliziert per EMF-ID. Tests: `NonOsgiManyToManyProxyTest` + `NonOsgiOneToManyProxyTest` (Proxy-Inhalt/URI, QueryCounter-Beweis, Resolution, bidi, Save-Roundtrip); Bestandstests auf ResourceSet-Read-Back umgestellt. Doku: getting-started.md „Many-valued references". | Performance-Variante | XL | ✅ Done |
| AP-49 | **Save-Pfad für Entities mit unresolved Non-Containment-Proxies:** `JPAResourceImpl.doSave` von `em.merge` auf ein `upsert`-Muster umgestellt: explizites `em.find(type, id)` liefert den Managed-Handle; Attribute werden kopiert; Non-Containment-Refs werden immer durch `em.getReference(type, refId)` ersetzt statt detached/proxy-EObjects im Slot zu lassen. Damit scannt EclipseLinks Commit-Phase keine unregistered Proxies mehr und versucht nicht mehr, sie als neu zu INSERTen. Zusätzlich: `EBasicIndirectionPolicy.objectIsInstantiated` behandelt nackte EObject-Proxies jetzt als instanziiert (verhindert ClassCastException aus Change-Tracking-Pfad). Neue Tests `testOneToOnePersistWithUnresolvedProxy` (Attribut-Update mit Proxy-Ref, ohne D anzufassen) und `testOneToOneReplaceRefUpdatesForeignKey` (Ref durch anderes Ziel ersetzen, FK wird neu geschrieben) — beide grün. Gesamter Build grün (100 Non-OSGi + 22 OSGi Tests). | Performance/Robustheit | M | ✅ Done |
| AP-50 | **PersistenceEngine-SPI entfernt (pre-release Bereinigung):** Die ungenutzte Engine-Indirektion ist vollständig entfernt — `PersistenceEngine`, `BasicPersistenceEngine`, `DefaultPersistenceEngine` (Package `…persistence.engine`) sowie die Command-Interfaces `Readable`/`Updateable`/`Deletable`/`Countable`. `PersistenceResource.getEngine()` aus dem Interface gestrichen; `JPAResourceImpl.getEngine()`-Override (warf `UnsupportedOperationException`) und zugehöriger Test entfernt. Begründung: kein Implementierer existiert, das Live-JPA-Backend arbeitet Resource-direkt gegen die EntityManagerFactory; neue Backends (Mongo) sollen den toten Contract nicht erben. Sinnvolle Erweiterungspunkte (QueryEngine, PrimaryKeyFactory, InputContentHandler) werden bei Bedarf als fokussierte Services neu eingeführt. | API-Bereinigung | S | ✅ Done |
| AP-48 | **Lazy-Loading für ManyToOne-Refs:** Erweitert AP-46 auf ManyToOne-Referenzen (singuläre Refs auf der „viele"-Seite einer bidirektionalen OneToMany). Historisch hatte `ReferenceConfigurator.processManyToOne` explizit `setUsesIndirection(false); dontUseIndirection();` aufgerufen — damit wurde unsere `EBasicIndirectionPolicy` für ManyToOne nie installiert und die Referenz eager geladen. Mit AP-48: zwei Zeilen entfernt, stattdessen `setIndirection(mapping, reference)` aufgerufen — analog zu processOneToOne. `ManyToOneMapping` erbt von `ObjectReferenceMapping`, also greift unser `valueFromQuery`-Override automatisch. Ein Test-Update nötig: `NonOsgiOneToManyTest.testOneToManyNonContainmentBidiEOpposite` nutzte `em.find(classEDescriptor.getJavaClass(), id)` und griff dann auf `findEEO.eGet(eClassAFeature)` zu — das ist jetzt ein Proxy ohne Resource, daher auf `findViaResource` + `EcoreUtil.resolve` umgestellt. Full-Build grün (94 Non-OSGi + 22 OSGi Tests). **Praktischer Nutzen:** Klassische N+1-Szenarien wie `orders.forEach(o → o.getOrderNumber())` laden nicht mehr 1000 Customer-Rows mit; Customer wird nur geladen, wenn `o.customer` tatsächlich abgefragt wird. | Performance/Skalierung | S | ✅ Done |
...
**Gesamtstand: 45 von 49 Arbeitspaketen erledigt (92 %)**
...
**Welle 14 — MappedSuperclass-Design:** AP-25 → ✅ Evaluiert + revidiert — MappedSuperclass wird jetzt gemeinsam mit AP-44 implementiert
...
**Welle 21 — Echtes Lazy-Loading:** AP-46 → ✅ Done (IndirectionPolicy.valueFromQuery liest FK aus Source-Row und erzeugt EclipseLink-managed Proxy direkt — echte DB-Latenz-Reduktion, Query-Zähler-Test beweist 1+1 statt 1+N), AP-48 → ✅ Done (ManyToOne-Refs jetzt analog lazy — zwei Zeilen Indirection-Disable in processManyToOne entfernt, setIndirection eingeplugt)
