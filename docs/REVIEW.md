...
| AP-44 | **TABLE_PER_CLASS + MappedSuperclass vollständig implementieren** (zusammengelegt mit AP-25 wegen 80 % gemeinsamer Pipeline). **Phase 1 — gemeinsame Maschinerie:** (a) `DirectToFieldMapping` auf Child-Descriptoren explizit auf die Child-Tabelle qualifizieren (`DatabaseField.setTableName(childTable)`), damit nicht die Default-Table-Mechanik mit dem Parent kollidiert (`Multiple writable mappings exist for [ROOT.attr]`); (b) `MappingProcessor.getEffectiveAttributes/Refs` spiegelt geerbte Features in Children bei TPC und MappedSuperclass; (c) `EntityProcessor.createIds()` erzeugt Child-spezifische IDs; (d) `configureDatabase` für abstrakten Root überdenken. **Phase 2a (TPC):** `setAbstract(true)` auf Root-Descriptor, `TablePerClassPolicy` mit `addParentDescriptor`/`addChildDescriptor`, polymorphe UNION-Queries; Referenz: EclipseLink's `InheritanceMetadata.addTablePerClassParentMappings()`. **Phase 2b (MappedSuperclass):** Detection via EAnnotation `mappedSuperclass="true"`; Root landet in `mappings.getMappedSuperclass()` statt `getEntity()` und bekommt bewusst keinen EclipseLink-Descriptor; Children sind unabhängige Entities mit eingebackenen Basis-Attributen. **Tests:** `NonOsgiInheritanceTablePerClassTest` (bereits vorhanden, `@Disabled`) + neue `NonOsgiMappedSuperclassTest` auf Basis von `NonOsgiInheritanceRoundtripBase`; für MappedSuperclass werden `testFindViaAbstractBaseReturnsConcreteSubtype` und `testPolymorphicQueryOverBase` übersprungen oder als Negativ-Assertion umgeschrieben. Modell-Fixture: neue Hierarchie `VehicleMs/CarMs/MotorcycleMs` in `data/model.ecore`. | F1-03, F1-05 | L | ❌ Offen |
...
**Welle 15 — Inheritance-Roundtrip + TPC/MappedSuperclass-Pipeline:** AP-43 → ⚠️ Teilweise (SINGLE_TABLE + JOINED Roundtrip-Tests grün; TPC-Bugs aufgedeckt, Teil-Fixes eingebaut), AP-44 → ❌ Offen (TPC und MappedSuperclass gemeinsam auf Basis geteilter Mapping-Pipeline)
