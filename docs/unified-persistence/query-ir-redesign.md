...
| `RootReference` | `from: →EClass`, `key: Expression`, `path: PropertyPath` | issue #241. A value read from ONE fixed object of another entity set — OData's `$root`. A **value** expression: composes with comparisons like `GeoDistance`. **No match yields null** and the enclosing comparison goes UNKNOWN under the 3VL rules (#94), which is what an empty scalar subquery does in SQL and what makes it composable without a second error path; **more than one match is an error**, never a first-row pick. Resolved **before translation** and inlined as a literal — one keyed read, which is why it is servable on mongo, whose `$match` cannot join across collections. The capability therefore belongs to the **resource** rather than to the expression evaluator, which is why the memory engine excludes `ROOT_REFERENCE` (it is handed one collection of objects and has no store to read from). A non-constant key would need a correlated subquery and is not served — declared, refused, not guessed |
