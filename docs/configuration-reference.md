...
| `OPTION_DELETE_IGNORE_REFERENCES` | `fennec.delete.ignore-references` | Boolean | `delete`, `execute` | Delete without checking or removing the references that still point at the deleted objects — no lookup, references may be left dangling (issue #347). Mongo only (`StoreFeature.DELETE_IGNORE_REFERENCES`); JPA refuses it. Contradicts the next option |
| `OPTION_DELETE_CLEAR_REFERENCES` | `fennec.delete.clear-references` | Boolean | `delete`, `execute` | Remove every reference to the deleted objects first, then delete (issue #347). Both backends (`StoreFeature.DELETE_CLEAR_REFERENCES`). A required single-valued reference still refuses |
